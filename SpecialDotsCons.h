...
void specialDotsCons(RGBTRIPLE**& rgb_in, RGBTRIPLE**& rgb_out, int imHeight, int imWidth, int threads, int ksize = 5)
{
...
#pragma omp parallel for reduction(max:max_v)
	for (int y = 0; y < imHeight; y++)
		for (int x = 0; x < imWidth; x++)
			if (R[y][x] > max_v) max_v = R[y][x];

#pragma omp parallel for reduction(min:min_v)
	for (int y = 0; y < imHeight; y++)
		for (int x = 0; x < imWidth; x++)
			if (R[y][x] < min_v) min_v = R[y][x];
	...
	do {
		...
		//if (i > 0) {
		int k = i + 1;
		while (k < harrisPoints.size()) {
			if (sqrt(pow(harrisPoints[i].X - harrisPoints[k].X, 2) + pow(harrisPoints[i].Y - harrisPoints[k].Y, 2)) < 20) {
				harrisPoints.erase(harrisPoints.begin() + k);
			}
			k++;
		};
		//for (int k = i + 1; k < harrisPoints.size(); k++) {
		//	if (sqrt(pow(harrisPoints[i].X - harrisPoints[k].X, 2) + pow(harrisPoints[i].Y - harrisPoints[k].Y, 2)) < 20) {
...
void specialDotsConsVector(RGBTRIPLE**& rgb_in, RGBTRIPLE**& rgb_out, int imHeight, int imWidth, int threads, int ksize = 5)
{
